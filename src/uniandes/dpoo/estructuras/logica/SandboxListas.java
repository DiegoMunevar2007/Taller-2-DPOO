...
public class SandboxListas
{
    ...
    public void reiniciarArregloEnteros( double[] valores )
    {
    	...
    	for (double elemento:valores) {
    		nuevaLista.addLast((int)elemento);
    	}
    ...
    public void reiniciarArregloCadenas( List<Object> objetos )
    {
    	...
    	for (Object elemento:objetos) {
    		nuevaLista.addLast((elemento.toString()));
    	}
    ...
    public void volverPositivos( )
    {
    	for (int i=0;i<this.listaEnteros.size();i++) {
    		if (this.listaEnteros.get(i)<0) {
    			...
    			this.listaEnteros.remove(i);
    			this.listaEnteros.add(i, elemento*-1);
    		}
    	}
    }
    ...
    /**
     * Cuenta cuántos valores dentro de la l     ista de enteros están repetidos.
     * @return La cantidad de enteos diferentes que aparecen más de una vez
     */
    public int contarEnterosRepetidos( )
    {
    	
    	Map<Integer,Integer> mapaRepetidos=new HashMap<>();
    	int contador=0;
    	for (int elemento:this.listaEnteros) {
    		if (!mapaRepetidos.containsKey(elemento)) {
    			mapaRepetidos.put(elemento, 1);
    		}
    		else {
    			mapaRepetidos.put(elemento, mapaRepetidos.get(elemento)+1);
    			
    		}
    	}
    	for (int valor:mapaRepetidos.values()) {
    		if (valor>1) {
    			contador++;
    		}
    	}
    ...
    public boolean compararArregloEnteros(int[] otroArreglo) {
        ...
        List<Integer> otroArreglo2 = new ArrayList<>();
        for (int elemento : otroArreglo) {
            otroArreglo2.add(elemento);
        }
        return copia.equals(otroArreglo2);
    }
    ...
    public void generarEnteros( int cantidad, int minimo, int maximo )
    {
    	...
    	for (int i=0;i<cantidad;i++) {
    		 nuevaLista.add(minimo + (int)(Math.random() * (maximo - minimo + 1)));
    	}
    	this.listaEnteros=nuevaLista;
    }
